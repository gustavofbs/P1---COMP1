...
try:
    import pytest_jsonreport as _  # type: ignore  # noqa: F401
except ImportError:
...
@pytest.fixture
def astf(src, parser):
    return lambda: filter_ast(parser(src))


@pytest.fixture
def ast_f(src_, parser):
    return lambda: filter_ast(parser(src_))


@pytest.fixture
def ast__f(src__, parser):
    return lambda: filter_ast(parser(src__))
